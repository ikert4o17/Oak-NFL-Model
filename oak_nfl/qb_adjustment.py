...
def qb_change_points(
    ...
    *,
    ...
    """Return the point adjustment for expected QB versus the team's baseline QB.

    Positive values improve the team's expected margin; negative values reduce it.
    The cap prevents unstable/small-sample quarterback estimates from producing
    implausibly large single-game moves.
    """
    if pd.isna(expected_qb_epa) or pd.isna(baseline_qb_epa):
        return 0.0
    raw = (float(expected_qb_epa) - float(baseline_qb_epa)) * epa_to_points
...
def add_qb_change_adjustments(
    ...
    *,
    ...
    required = {
        "predicted_home_margin", home_expected_col, home_baseline_col,
        away_expected_col, away_baseline_col,
    }
    ...
    out["home_qb_change_points"] = [
        qb_change_points(e, b, max_adjustment=max_adjustment)
