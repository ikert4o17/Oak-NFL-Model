...
def prepare():
    ...
    if "touchdown" not in p.columns or "field_goal_result" not in p.columns: raise ValueError("PBP missing scoring columns")
    p["off_td"]=(pd.to_numeric(p.touchdown,errors="coerce").fillna(0).eq(1)&p.play_type.isin(["pass","run","qb_kneel"])).astype(int)
    p["fg_made"]=p.field_goal_result.fillna("").astype(str).str.lower().eq("made").astype(int)
    score=p.groupby(["game_id","posteam"],as_index=False).agg(off_td=("off_td","sum"),fg_made=("fg_made","sum")); score["off_points"]=7*score.off_td+3*score.fg_made
...
def features(g,windows=(3,4,5,6,8)):
    ...
    for r in g.sort_values(["season","week","game_id"]).itertuples(index=False):
        ...
        rows.append(out)
        hist[r.home_team].append((r.home_off_points,r.home_yards,r.away_off_points,r.away_yards)); hist[r.away_team].append((r.away_off_points,r.away_yards,r.home_off_points,r.home_yards))
...
def run():
    ...
    sets={}
    for w in [3,4,5,6,8]:
        sets[f"clean_ppa_{w}"]=[f"home_clean_ppa100_{w}",f"away_clean_ppa100_{w}"]; sets[f"clean_dpa_{w}"]=[f"home_clean_dpa100_{w}",f"away_clean_dpa100_{w}"]; sets[f"clean_both_{w}"]=sets[f"clean_ppa_{w}"]+sets[f"clean_dpa_{w}"]
    # Targeted mixed-window tournament: offense appears slower-moving than defensive scoring suppression.
    for pw in [4,5,6,8]:
        for dw in [3,4,5]:
            sets[f"clean_ppa{pw}_dpa{dw}"]=[f"home_clean_ppa100_{pw}",f"away_clean_ppa100_{pw}",f"home_clean_dpa100_{dw}",f"away_clean_dpa100_{dw}"]
    rows=[{"model":n,**metrics(ho.actual_total,fit(tr,ho,c))} for n,c in sets.items()]
    res=pd.DataFrame(rows).sort_values(["mae","rmse"]); print(res.to_string(index=False))
    print("\n=== TOP 8 YEAR STABILITY ===")
