...
GROUPS = {
    "ol": {"OT", "IOL"}, "receivers": {"WR", "TE"}, "rb": {"RB"},
    "pass_rush": {"EDGE", "IDL"}, "lb": {"LB"}, "secondary": {"CB", "S"},
    "all_non_qb": set(POSITION_POINT_CAPS),
}


def _canonical_injuries(raw: pd.DataFrame) -> pd.DataFrame:
    data = raw.loc[raw["report_status"].notna() & raw["report_status"].astype(str).str.strip().ne("")].copy()
    normalized = normalize_injury_feed(data, column_map={
        "gsis_id": "player_id", "full_name": "player_name", "position": "position_group",
        "report_status": "status", "date_modified": "report_date",
    }, source="nflverse_injuries")
    return latest_weekly_status(normalized)


def _weekly_team_adjustments(availability: pd.DataFrame, *, positions: set[str], scale: float, team_cap: float) -> pd.DataFrame:
    frame = availability[availability["position_group"].isin(positions)].copy()
    frame = frame[frame["position_group"].ne("QB")]
    frame["absence_points"] = [scale * player_absence_points(pos, value, status) for pos, value, status in zip(frame.position_group, frame.player_value, frame.status)]
    grouped = frame.groupby(["season", "week", "team"], as_index=False)["absence_points"].sum().rename(columns={"absence_points": "personnel_points"})
...
def _apply_weekly_adjustments(predictions: pd.DataFrame, adjustments: pd.DataFrame) -> pd.DataFrame:
    home = adjustments.rename(columns={"team": "home_team", "personnel_points": "home_personnel_points"})
    away = adjustments.rename(columns={"team": "away_team", "personnel_points": "away_personnel_points"})
    out = predictions.merge(home, on=["season", "week", "home_team"], how="left").merge(away, on=["season", "week", "away_team"], how="left")
    out[["home_personnel_points", "away_personnel_points"]] = out[["home_personnel_points", "away_personnel_points"]].fillna(0.0)
    out["predicted_home_margin"] = out["predicted_home_margin"] + out["home_personnel_points"] - out["away_personnel_points"]
    return out


def _coverage_diagnostics(availability: pd.DataFrame, values: pd.DataFrame) -> None:
    ...
    # For every injury row, look backward for the player's latest known pregame
    # value in the player-value table. This is diagnostic only and never enters
    # the game prediction itself.
    ...
    availability["ever_known_snap_player"] = availability["player_id"].astype(str).isin(known_ids)
    print(f"injury rows belonging to a known snap player: {availability['ever_known_snap_player'].mean():.4f}")
    known = availability[availability["ever_known_snap_player"]]
    if len(known):
        print(f"coverage among known snap players: {known['player_value'].gt(0).mean():.4f}")
    ...
    buckets["week_bucket"] = pd.cut(buckets["week"], bins=[0, 3, 8, 13, 30], labels=["W1-3", "W4-8", "W9-13", "W14+"])
    by_week = buckets.groupby("week_bucket", observed=True).agg(
        injury_rows=("player_name", "size"), matched=("player_value", lambda s: int(s.gt(0).sum()))
    )
...
def run() -> None:
    ...
    holdout = v5[v5["season"].between(2023, 2024)].dropna(subset=["predicted_home_margin", "actual_home_margin"])
    ...
    for group, positions in GROUPS.items():
        for scale in [0.25, 0.50, 0.75, 1.00]:
            for cap in [1.0, 2.0, 3.0]:
                adjustments = _weekly_team_adjustments(availability, positions=positions, scale=scale, team_cap=cap)
                metrics = evaluate_margin_predictions(_apply_weekly_adjustments(holdout, adjustments))
